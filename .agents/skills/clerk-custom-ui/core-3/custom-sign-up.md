...
### Create (identifier-first OTP flows)

Use `create()` to start a sign-up with an identifier before sending an OTP code.
...
After `create()`, send the appropriate verification code with `signUp.verifications.sendPhoneCode()` or `signUp.verifications.sendEmailCode()`.
